...
pub fn get_avatar(
      ...
      let path = binding.replace("avatar://", "").replace("/", "").replace("#instant=reload", "");


      let entries = fs::read_dir("/usr/flareos/avatars")?;
      let mut found = false;
      ...
      let mut file = std::fs::File::open(format!("/usr/flareos/avatars/{}", &path))?;
      ...
      let http_response = if let Some(range_header) = request.headers().get("range") {
        ...
        if ranges.len() == 1 {
          ...
          resp = resp.header("Cache-Control", "no-store");
